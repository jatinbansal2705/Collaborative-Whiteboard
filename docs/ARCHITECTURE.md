...
### ADR-0021: Accessibility (WCAG AA) as a hard acceptance gate

- **Status**: Accepted
- **Context**: Phase 14 targets WCAG AA across the whole product, including a canvas whose tools are fundamentally pointer-driven, plus complex Radix overlay dialogs and realtime surfaces that mutate without full page navigation.
- **Decision**: Accessibility is treated as a release gate, not a checklist. The document gets a visible-focus skip link to the board; focus management is explicit in every dialog/menu (Radix focus trap + restore); the Konva stage exposes a single-focus surface with `aria-label`, keyboard canvas commands (select/delete/move via arrow keys, Enter to insert, Del to delete), and a hidden accessible "scene tree" list for screen readers when selection is active. Color uses the semantic oklch tokens designed to pass AA contrast (verified by Lighthouse `color-contrast` assertions and manual review); `prefers-reduced-motion` disables pan/draw animations and transitions; all icon-only controls carry accessible labels; notification banners use `role="status"`/`aria-live`. Every interactive element is keyboard reachable and the tab order follows the visual layout. Automated checks run via Lighthouse assertions (`aria-allowed-attr`, `button-name`, `color-contrast`, `focus-traps`) plus `axe-core` style unit assertions where practical.
- **Consequences**: The board is usable by keyboard and screen reader for core flows (select, draw, delete, text editing, panel navigation, sharing, chat). A11y regressions fail Lighthouse CI and the unit suite. Pointer-only interactions are rejected at review.

### ADR-0022: Responsive board UI with bottom-sheet panels

- **Status**: Accepted
- **Context**: Phase 12's side panels (layers, comments, chat, share) and toolbars were designed for a desktop viewport; the same board must remain usable on tablet and phone widths.
- **Decision**: Breakpoints follow the PRD (desktop `≥1024px`, tablet `768–1024px`, mobile `<768px`). Below `1024px` the board toolbars compress and the side panels convert to bottom sheets that preserve scroll position and state on open/close, so context is never lost between panel toggles. The canvas remains the primary surface at all widths; zoom/pan are touch-gesture friendly and the minimap stays reachable. Panel state (which panel, size) is kept in the store so layout changes are smooth and lossless.
- **Consequences**: Board collaboration works on tablets and phones; panel content (lists, forms, chat) is keyboard/AT reachable in the bottom-sheet layout as well as the desktop split-pane layout.

### ADR-0023: Test strategy — unit + socket harness + e2e + load

- **Status**: Accepted
- **Context**: By the end of Phase 13 the web suite and core API unit tests exist, but Phase 2–13 shipped features with thin realtime coverage and no load or end-to-end verification.
